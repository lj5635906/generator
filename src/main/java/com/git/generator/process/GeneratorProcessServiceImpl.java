...
public class GeneratorProcessServiceImpl extends AbstractGeneratorProcess {
    ...
    protected void action(Configuration config, Map<String, Table> table, Map<String, List<EntityProperty>> tableColumn) throws Exception {
        // 生成代码目录
        String target = GeneratorConfig.target + GeneratorConstant.PACKAGE_NAME_SERVICE + GeneratorConstant.FILE_DELIMITER + GeneratorConstant.PACKAGE_NAME_SERVICE_IMPL + GeneratorConstant.FILE_DELIMITER;
        ...
        while (entities.hasNext()) {
            ...
            data.put("serviceImplFullPackageName", GeneratorUtil.getServiceImplFullClassName(GeneratorConfig.packageName, GeneratorConfig.moduleName));
